...
**2026.05 ~ 진행중 · 어댑터 엔지니어 겸 기술 조율 리드 (총 6인) · 2026.07.15 오픈**

> 서버 설정을 직접 변경할 수 없는 금융권 망분리 환경에서 **비침습적 진단 기법으로 SSO 장애의 근본 원인을 특정**하고,  
> **5개 이상 외부 조직 간 기술 조율을 리드**하여 잔여 이슈 없이 오픈 일정을 지켜냈습니다.

#### 문제 상황
- 금융권 접근통제 정책상 운영 서버의 설정 변경·패키지 설치가 불가능한 환경에서 SSO 어댑터(Spring Boot) 초기화가 실패
- SaaS 개발사(PM only), 그룹웨어 인프라사(Dooray), 하나증권 인프라팀(이중화·방화벽 별도 조직), VPN사(대신네트워크), 기존 DRM사(Softcamp) — **5개 조직이 각자 다른 정보를 갖고 있어** 원인 규명이 정체

#### 해결 과정
- 운영 서버 SSO 초기화 실패의 근본 원인을 **DNS 미해석 + 방화벽 미개방의 복합 요인**으로 특정
- 권한 제약 환경에서 `-Djdk.net.hosts.file` 기반 사전 검증 등 **서버 상태를 변경하지 않는 비침습적 진단 기법**으로 추측이 아닌 **증거**를 확보 — 이를 근거로 인프라팀의 방화벽·DNS 조치를 이끌어냄
- 다자간 이해관계자의 상이한 정보를 취합·검증·조율하는 **기술 리드** 역할 수행
- 고객사 IT 현업과 주간 정례 미팅으로 이슈·일정을 직접 협의

#### 성과
- 전 테스트 통과, **잔여 이슈 0건으로 오픈 일정(7/15) 확정**
- 조직 간 정보 불일치로 정체되던 인프라 이슈(DNS/방화벽/VPN)를 주도적으로 해소하여 **일정 지연 없이 진행**
...
- 계정 분리 여부, 인증서 발급 등 비용·일정에 직결되는 아키텍처 의사결정이 미확정
...
- **아키텍처 의사결정 주도**: 계정 분리 대신 단일 계정 + 네이밍/태그 분리 방식을 제안·확정하여 cross-account trust relationship 관리 비용을 원천 제거
- **인증서 비용 회피 판단**: 기존 `*.kbs.co.kr` 와일드카드 인증서가 커버되는 `devg.kbs.co.kr` 도메인 체계를 검토·제안 — 추가 인증서 발급 비용 없이 진행하고, 폐쇄망 환경의 인증서 신뢰 오류 리스크까지 사전 차단
- KBS 인프라팀과 도메인 발급·DNS(A/CNAME) 신청·일정 협의를 직접 수행, 무리한 일정에는 근거를 들어 재조정을 건의하고 관철
