...
- **현재는 VM + systemd로 구동하고, 쿠버네티스는 확장 가능성으로 열어두었습니다** — 대상 서비스가 실시간 고객 트래픽을 받고 있어 이관 자체의 리스크를 낮추는 것이 우선이었고, 한 호스트는 가용 RAM이 6GB 수준이라 컨트롤플레인 오버헤드(약 4GB)를 감당할 여유가 없었습니다. **현재의 자원과 리스크 조건에 맞는 방식**을 택하되, Dockerfile·매니페스트는 미리 준비해 **이후 systemd를 Pod로 교체하는 전환 경로**를 남겨두었습니다
