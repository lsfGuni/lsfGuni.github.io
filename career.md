...
- **① 비용 구조 — FinOps 관점의 온프레미스 이관.** 실사용자 규모에 비해 AWS 자원이 **과다 프로비저닝된 상태**라고 판단하고, 사내 유휴 물리서버로 워크로드를 옮기는 방향을 잡았습니다
- **② 안전한 이관과 운영.** 실서비스 트래픽을 받는 중이라 **중단 없이 옮기고, 옮긴 뒤에도 안정적으로 유지**되어야 했습니다
