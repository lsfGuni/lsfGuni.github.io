...
**2026.05 ~ 진행중 · 어댑터 엔지니어 (총 6인) · 2026.07.15 오픈**

> 외부망 SaaS부터 내부 SSO·DRM 애플리케이션까지, **업체가 모두 다른 시스템이 다단계로 이어지는 금융권 망분리 환경**에서  
> **구간별 통신 검증으로 연동 장애의 원인을 특정**하고 다자간 업무 조율로 해결했습니다.

#### 문제 상황
- 그룹웨어 SaaS 도입을 위해 SSO·DRM 어댑터를 구축했으나, **Dooray(외부망) → VPN → 하나증권 인프라 → SSO·DRM 어댑터 → 내부 SSO·DRM 애플리케이션**으로 이어지는 다단계 연동 구간에서 통신 장애가 발생
