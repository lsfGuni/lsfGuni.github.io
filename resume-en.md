...
- **Migrated the platform off AWS onto in-house hardware — monthly spend down 78%.** Replaced the entire AWS edge (Route 53, 8 ALBs, WAF, reverse-proxy EC2) with **Cloudflare Tunnel**, eliminating inbound firewall exposure. Migration order followed risk, rollback was secured before every cutover — 8 domain cutovers, zero regressions
...
- **Built an operations set that an AI session can run incident response from** — a diagnostic script and nine symptom-based runbooks packaged with entry rules, so an operator typing "scan is down" is still walked through establish facts → isolate cause → act
- **Built a Kubernetes (k3s) isolation platform** — one Job per agent execution with a dedicated namespace, ServiceAccount and RBAC, deployed declaratively via ArgoCD app-of-apps across 14 Helm charts. Verified end to end *(not production operating experience)*
