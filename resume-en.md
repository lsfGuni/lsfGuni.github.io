...
- **Set up the handover so an AI session can run incident response** — a diagnostic script and nine symptom-based runbooks packaged with entry rules, so a successor typing "scan is down" is still walked through establish facts → isolate cause → act
