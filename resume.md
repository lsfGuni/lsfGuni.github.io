...
- **AWS 비용 절감을 위한 온프레미스 이관(FinOps) — 월 고정비 78% 절감** — 진입 계층(Route 53 + ALB 8개 + WAF + 리버스프록시 EC2)을 **Cloudflare Tunnel**로 전면 이전해 방화벽 인바운드 노출을 없애고, 서비스를 사내로 전량 이관. 이관 순번을 리스크 기준으로 설계하고 롤백 경로를 먼저 확보한 뒤 도메인 단위로 전환 (8건 전환, 회귀 0건)
...
- **AI 세션이 장애 대응을 수행하는 운영 세트 구축** — 진단 스크립트 + 증상별 런북 9종을 Claude Code 진입 규칙과 묶어, 담당자가 "scan이 안 돼" 한 줄로 시작해도 사실 확보 → 원인 특정 → 조치 순서를 밟게 함
- **Kubernetes(k3s) 격리 실행 인프라 구축 완료** — 에이전트 실행 1건 = Job 1개, 전용 네임스페이스·ServiceAccount·RBAC, ArgoCD app-of-apps로 Helm 차트 14종 선언적 배포. 관통 검증 완료 *(상용 트래픽 운영 경험은 아님)*
...
- NIPA 블록체인 정부과제 — **의약품 이력관리 시스템** 백엔드·인프라 담당
...
- **삼성디스플레이 폐쇄망 AI 플랫폼 구축** (2026.05~06, 인프라 담당) — 인터넷 차단·USB 단방향 반입 환경에 AI 서비스 5종 배포. ARM↔x86_64 아키텍처 불일치 등 반입 실패 요인을 제거해 **1사이클 5시간+ → 30분(약 90% 단축)**. 9월 후속 단계 확정
- **KBS 통합재난방송시스템 STG 인프라 구축** (2026.04~06) — 검증 환경이 없던 대국민 서비스에 운영과 동등한 스테이징 신규 구축. ECS Fargate + CodeDeploy Blue/Green, CloudFront 7경로 분기, IAM 최소권한. **PRD-STG 전 항목 CLI 대조 검증, 불일치 0건**
