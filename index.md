...
<script>
  // 펼치기/접기: 각 collapse 요소에 직접 이벤트 바인딩
  $('#projectAccordion .collapse').on('shown.bs.collapse', function() {
    var id = $(this).attr('id');
    var toggle = $('[data-target="#' + id + '"]');
    toggle.find('.toggle-text').text('접기');
    toggle.attr('aria-expanded', 'true');
  });
  $('#projectAccordion .collapse').on('hidden.bs.collapse', function() {
    var id = $(this).attr('id');
    var toggle = $('[data-target="#' + id + '"]');
