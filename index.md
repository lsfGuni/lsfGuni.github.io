...
<div class="strength-item">
  <strong>Kubernetes &amp; GitOps</strong> — 쿠버네티스 클러스터 직접 구축·운영(경량 배포판 k3s, Linkerd mTLS, Sealed Secrets), ArgoCD app-of-apps로 Helm 차트 14종·앱 19개 관리, Job + RBAC + egress NetworkPolicy 기반 워크로드 격리. 관리형(EKS·GKE)이 아닌 self-managed 클러스터를 직접 구성한 경험
</div>
<div class="strength-item">
...
<div class="exp-card">
  ...
  <ul>
    ...
    <li>사내 AI 에이전트 실행 플랫폼 인프라 구축(2026.05~): k3s 클러스터 + ArgoCD GitOps(Helm 차트 14종·앱 19개), 에이전트 실행 1건 = Job 1개 격리 + egress NetworkPolicy 화이트리스트로 시크릿 반출 차단, Linkerd mTLS, self-host 레지스트리 3종</li>
