...
<h6>백엔드 개발</h6>
  <ul>
    <li>방문자 등록, 승인, 출입, 이력 관리 등 핵심 비즈니스 로직 설계 및 구현</li>
    <li>카드사 연동 API 개발을 통한 발급/회수 상태 실시간 동기화</li>
    <li>TBL31, TBL32 기반 UNION SQL 구조 분석 및 데이터 불일치 문제 해결</li>
    <li>VISITOR_RESNO(표시용)와 REAL_RESNO(실제 식별키) 불일치 문제를 분석하여 데이터 정합성 개선</li>
    <li>Mobile 카드 데이터가 잘못 포함되는 문제를 SQL 레벨 필터링으로 해결</li>
  </ul>

<h6>프론트엔드 개발</h6>
  <ul>
    <li>JSP + JWORK Grid 기반 화면 개발</li>
    <li>JavaScript를 활용한 동적 UI 처리 및 입력 검증 로직 구현</li>
    <li>hidden 필드 매핑 오류(VISITOR_RESNO → REAL_RESNO)를 수정하여 실운전자 정보 미표시 문제 해결</li>
    <li>데이터 상태(회수 여부, 실운전자 존재 여부)에 따른 라디오 선택 제어 로직 구현</li>
  </ul>

<h6>문제 해결 및 시스템 개선</h6>
  <ul>
    <li>실운전자 등록 후 화면에 표시되지 않는 문제를 데이터 흐름(DB → SQL → JSP) 추적으로 해결</li>
    <li>UI 문제로 보이던 이슈를 SQL/데이터 계층 문제로 식별하여 근본 원인 해결</li>
    <li>JSP 필터링 대신 SQL 필터링으로 로직을 이동시켜 전체 시스템 일관성 확보</li>
    <li>조인 조건 및 필터 차이로 발생하는 데이터 건수 불일치 문제 분석 및 개선</li>
  </ul>
...
<h5>성과</h5>
  <ul>
    <li>실제 운영 환경에서 발생한 데이터 불일치 문제를 해결하여 시스템 신뢰성 향상</li>
