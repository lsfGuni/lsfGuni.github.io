...
<style>
  ...
  .print-btn:hover { border-color: #008AFF; color: #008AFF; background: #f8f9ff; }
  ...
  /* PDF 인쇄용 */
  @media print {
    .intro-header, nav, footer, .contact-bar, .toggle-wrap, .print-btn { display: none !important; }
...
<script>
  $(document).ready(function() {
    ...
    // 펼치기/접기: jQuery slideToggle 직접 제어
    $('.project-toggle').on('click', function() {
      ...
      if (isOpen) {
        ...
        $('.project-toggle[aria-expanded="true"]').each(function() {
          ...
          $other.find('.toggle-text').text('펼치기');
          $other.attr('aria-expanded', 'false');
        });
        // 현재 패널 열기
        $target.slideDown(300);
        $this.find('.toggle-text').text('접기');
