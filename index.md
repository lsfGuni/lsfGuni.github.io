...
<div id="projectAccordion">
  ...
  <div id="proj-berith-ops" class="project-detail">
    ...
    <p>소스를 읽어 보니 이 캐시는 저장소가 아니라 <strong>분산 락 코디네이터</strong>였습니다. ZooKeeper와 교체 가능한 자리였고, 실제로 저장된 키는 0개였습니다.</p>
    <ul>
    ...
    <p>BaaS API를 2노드로 늘린 직후 신규 노드가 8분간 멈췄습니다. 스레드 덤프와 소켓 상태를 따라가 보니 <strong>기존 노드에만 있던 DNAT 규칙</strong>이 원인이었습니다.</p>
    <ul>
      <li>같은 조사에서 <strong>신규 노드에 방화벽 정책이 빠져 있는 것</strong>을 발견 — 원격 작업이라 240초 자동 롤백 타이머를 걸어 두고 규칙 적용</li>
      <li>연결·대기 타임아웃이 모두 없어 <strong>노드 하나가 멈추면 처리기 전체가 멈추는</strong> 구조적 결함도 확인해 수정 과제로 등록</li>
      <li><strong>복제했다고 믿은 노드가 실제로는 달랐습니다.</strong> 아티팩트는 해시까지 맞췄지만 OS 설정은 비교 대상이 아니었습니다</li>
    </ul>
