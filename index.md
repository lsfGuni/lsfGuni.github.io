...
<div id="projectAccordion">
  ...
  <div id="proj-berith-ops" class="project-detail">
    ...
    <h6>설계 판단</h6>
    <ul>
      <li><strong>현재는 VM + systemd로 구동, 쿠버네티스는 확장 가능성으로 검토</strong> — 대상 서비스가 실시간 고객 트래픽을 받고 있어 이관 리스크를 낮추는 것이 우선이었고, 한 호스트는 가용 RAM 6GB 수준이라 컨트롤플레인 오버헤드(약 4GB)를 감당할 여유가 없었음. <strong>현재 자원과 리스크 조건에 맞는 방식</strong>을 택하되, Dockerfile·매니페스트는 미리 준비해 <strong>이후 systemd를 Pod로 교체하는 전환 경로</strong>를 확보</li>
