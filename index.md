...
Spring 백엔드 개발로 시작하여 Linux 서버 구축·운영, 하이브리드 인프라, CI/CD 파이프라인 구축·개선, 모니터링·알림 체계(관측성), 보안 대응까지 확장했습니다. 장애 발생 시 근본 원인 분석(RCA) → 해결 → 고도화 → 문서화까지 이어가며, 재현 가능한 운영 체계를 만듭니다. 현재는 Kubernetes(k3s·kubeadm)와 Terraform·Ansible 기반 IaC로 운영 방식을 전환하고 있습니다.
...
<span class="badge-skill infra">gVisor</span>
...
<div class="strength-item">
  <strong>Kubernetes & IaC (전환 진행 중)</strong> — kubeadm·k3s 클러스터 직접 구축, 사내 AI 에이전트 실행 플랫폼(k3s + gVisor, RuntimeClass 격리) 참여, Proxmox 3대 홈랩을 Terraform + Ansible로 코드화
</div>
<div class="strength-item">
...
<div class="exp-card">
  ...
  <ul>
    ...
    <li>k3s·gVisor 기반 AI 에이전트 실행 플랫폼 구축 참여 (RuntimeClass 컨테이너 격리, 진행 중)</li>
    <li>하나증권 그룹웨어 어댑터(2026.05~): 금융권 망분리 환경 SSO·DRM 어댑터 구축, 다단계 연동 구간의 통신 장애 진단 및 다자간 업무 조율</li>
...
<div id="projectAccordion">
  ...
  <div class="project-toggle" data-target="#proj-ai-agent" aria-expanded="false">
    <span class="toggle-wrap"><span class="toggle-arrow">▼</span></span>
    <h4>🤖 AI 에이전트 실행 플랫폼 — k3s·gVisor 샌드박스</h4>
    <p class="proj-meta">진행 중 · 사내 프로젝트 · 샌드박스 실행 환경 설계·구축</p>
    <p class="proj-summary">스스로 코드를 수정하는 멀티에이전트가 호스트 파일을 건드려 자기 자신을 망가뜨리는 문제를, k3s 위 gVisor 샌드박스(RuntimeClass) 격리로 해결하는 실행 플랫폼 설계·구축</p>
    <div>
      <span class="tech-tag">Kubernetes</span>
      <span class="tech-tag">k3s</span>
      <span class="tech-tag">gVisor</span>
      <span class="tech-tag">RuntimeClass</span>
      <span class="tech-tag">Sandbox</span>
    </div>
  </div>
  <div id="proj-ai-agent" class="project-detail">
    <h5>Background</h5>
    <p>이전 POC에서 멀티에이전트가 자율적으로 코드를 수정·실행하는 과정에서, 격리 공간이 없어 <strong>에이전트가 호스트 파일을 변경해 자기 자신의 코드와 실행 환경을 훼손</strong>하는 부작용이 발생했습니다. 일반 컨테이너 런타임 수준의 격리로는 부족하다고 판단해, 커널 수준 샌드박스 격리 아키텍처를 설계·구축하고 있습니다.</p>

    <h5>What I'm Doing</h5>
    <ul>
      <li>k3s 클러스터 위 gVisor 샌드박스 실행 환경 구성</li>
      <li>RuntimeClass로 에이전트 워크로드만 샌드박스 런타임에서 실행하도록 분리</li>
      <li>에이전트 실행 환경의 격리 아키텍처 설계·구축 (초기 단계)</li>
    </ul>
  </div>
  ...
  <div class="project-toggle" data-target="#proj-hana" aria-expanded="false">
    ...
    <p class="proj-meta">2026.05 ~ 진행중 (2026.07.15 오픈) · 어댑터 엔지니어 (총 6인)</p>
  ...
  <div id="proj-hana" class="project-detail">
    <h5>Background</h5>
    <p>하나증권 그룹웨어(SaaS) 도입 프로젝트에서 SSO·DRM 어댑터의 구축과 연동 트러블슈팅을 담당했습니다. <strong>Dooray(외부망) → VPN → 하나증권 인프라 → SSO·DRM 어댑터 → 내부 SSO·DRM 애플리케이션</strong>으로 이어지는 다단계 연동 구간에서 통신 장애가 발생했는데, 구간마다 담당 업체가 모두 달라 어느 한 곳도 전체 흐름을 알지 못하는 상태로 원인 규명이 정체되는 상황이었습니다.</p>
    ...
    <h5>Outcome</h5>
    <ul>
      <li>전 테스트 통과, <strong>잔여 이슈 없이 오픈(7/15)</strong></li>
