...
백엔드 개발로 시작하여 Linux 서버 운영, 하이브리드 인프라, CI/CD, 모니터링, 보안 대응까지 확장했습니다. 문제 발생 시 원인 분석 → 해결 → 고도화 → 문서화까지 이어가며, 재현 가능한 운영 체계를 만듭니다.
...
<div id="projectAccordion">
  ...
  <!-- 프로젝트 0-1: 삼성디스플레이 폐쇄망 AI POC (2026.05 ~ 2026.06) -->
  <div class="project-toggle" data-target="#proj-sdc-poc" aria-expanded="false">
    ...
    <h4>🔒 삼성디스플레이 폐쇄망 AI POC 인프라 구축</h4>
    <p class="proj-meta">2026.05 ~ 2026.06 · 폐쇄망 인프라 담당 (인프라 1인 / 총 5인)</p>
    <p class="proj-summary">인터넷 완전 차단 폐쇄망에 LLM 기반 5개 서비스 스택을 오프라인 배포. 반입-배포 1사이클 1일 → 30분 단축, POC 성공 완료</p>
    <div>
  ...
  <div id="proj-sdc-poc" class="project-detail">
    <h5>Background</h5>
    <p>인터넷이 완전히 차단된 삼성디스플레이 폐쇄망 환경에서 LLM 기반 AI 애플리케이션 스택(LLM 게이트웨이, Text-to-SQL 엔진, STT, 임베딩 서버 등 5개 서비스)의 오프라인 배포 체계를 설계하고 POC를 완수해야 했습니다. CI/CD 구성이 불가능하고, 모든 산출물은 물리 반입으로만 전달할 수 있는 환경이었습니다.</p>
  ...
  <!-- 프로젝트 0-2: 하나증권 그룹웨어 어댑터 (2026.05 ~ 진행중) -->
  <div class="project-toggle" data-target="#proj-hana" aria-expanded="false">
    <span class="toggle-wrap"><span class="toggle-arrow">▼</span></span>
    <h4>🏦 하나금융그룹(하나증권) 그룹웨어 어댑터 엔지니어링</h4>
    <p class="proj-meta">2026.05 ~ 진행중 (2026.07.15 오픈 확정) · 어댑터 엔지니어 (총 6인)</p>
    <p class="proj-summary">금융권 망분리 환경에서 SSO·DRM·인사연동 어댑터 배포와 연동 트러블슈팅 담당, 5개 이상 외부 조직 간 기술 조율 리드</p>
    <div>
  ...
  <div id="proj-hana" class="project-detail">
    <h5>Background</h5>
    <p>하나증권 그룹웨어(SaaS) 도입 프로젝트에서 SSO·DRM·인사연동 어댑터의 배포와 연동 트러블슈팅을 담당했습니다. 금융권 망분리·접근통제 환경에서 SaaS 개발사, 그룹웨어 인프라사, 고객사 인프라팀, VPN사, DRM사 등 다수 외부 조직의 정보가 서로 달라 인프라 이슈가 정체되는 상황이었습니다.</p>

    <h5>What I Did</h5>
    <h6>연동 장애 진단</h6>
    <ul>
      <li>금융권 망분리·접근통제 환경에서 SSO 어댑터(Spring Boot) 배포 및 연동 장애 분석</li>
      <li>운영 서버 SSO 초기화 실패의 근본 원인을 <strong>DNS 미해석 + 방화벽 미개방</strong>으로 특정</li>
      <li>권한 제약 환경에서 <code>-Djdk.net.hosts.file</code> 기반 사전 검증 등 비침습적 진단 기법으로 인프라팀 조치 요청의 근거 확보</li>
    </ul>
    <h6>다자간 기술 조율 리드</h6>
    <ul>
      <li>SaaS 개발사(PM only), 그룹웨어 인프라사(Dooray), 하나증권 인프라팀(이중화/방화벽 별도 조직), VPN사(대신네트워크), 기존 DRM사(Softcamp) 등 다자간 이해관계자의 상이한 정보를 취합·조율</li>
      <li>고객사 IT 현업과 주간 정례 미팅으로 이슈·일정 직접 협의</li>
    </ul>

    <h5>Outcome</h5>
    <ul>
      <li>전 테스트 통과, 잔여 이슈 없이 <strong>오픈 일정(7/15) 확정</strong></li>
      <li>조직 간 정보 불일치로 정체되던 인프라 이슈(DNS/방화벽/VPN 설정)를 주도적으로 해소하여 일정 지연 방지</li>
      <li>금융권 보안 제약 하에서의 장애 진단·조율 프로세스 경험 확보</li>
    </ul>
  </div>
  ...
  <div class="project-toggle" data-target="#proj-kbs-stg" aria-expanded="false">
    ...
    <p class="proj-summary">AWS 스테이징 환경을 단독 설계·구축, 실 작업 2주 내 전 리소스 구축 완료 및 Jenkins 배포 테스트까지 검증</p>
    <div>
  ...
  <div id="proj-kbs-stg" class="project-detail">
    ...
    <h6>아키텍처 의사결정 주도</h6>
    <ul>
      <li>계정 분리 대신 단일 계정 + 네이밍/태그 분리 방식 제안·확정 (계정 분리 시 발생하는 trust relationship 추가 작업 회피)</li>
      <li>기존 <code>*.kbs.co.kr</code> 와일드카드 인증서가 커버되는 <code>devg.kbs.co.kr</code> 도메인 체계를 검토·제안하여 추가 인증서 발급 없이 진행, 폐쇄망 환경의 인증서 신뢰 오류 리스크까지 사전 검토</li>
    </ul>
    <h6>고객사 직접 협의</h6>
    <ul>
